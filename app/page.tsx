...
const services = [
  ...
  },
  {
    ...
    icon: <Lightbulb className="h-6 w-6 text-primary" aria-hidden="true" />,
  },
]
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {services.map((service) => (
              <Card key={service.title} className="flex flex-col justify-between border-white/10 bg-white/[0.06] p-6">
                ...
                <p className="text-xs uppercase tracking-wide text-white/50">Available as advisory or embedded leadership</p>
              </Card>
            ))}
          </div>
        </section>
